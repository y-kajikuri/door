...
public class BsMemberWithdrawalCB extends AbstractConditionBean {
    ...
    protected MemberWithdrawalCQ createLocalCQ() {
        return xcreateCQ(null, getSqlClause(), getSqlClause().getBasePointAliasName(), 0);
    }

    protected MemberWithdrawalCQ xcreateCQ(ConditionQuery childQuery, SqlClause sqlClause, String aliasName, int nestLevel) {
        MemberWithdrawalCQ cq = xnewCQ(childQuery, sqlClause, aliasName, nestLevel);
        cq.xsetBaseCB(this);
        return cq;
    }

    protected MemberWithdrawalCQ xnewCQ(ConditionQuery childQuery, SqlClause sqlClause, String aliasName, int nestLevel) {
     ...
     */
    public void union(UnionQuery<MemberWithdrawalCB> unionCBLambda) {
     ...
     */
    public void unionAll(UnionQuery<MemberWithdrawalCB> unionCBLambda) {
        final MemberWithdrawalCB cb = new MemberWithdrawalCB(); cb.xsetupForUnion(this); xsyncUQ(cb);
        try { lock(); unionCBLambda.query(cb); } finally { unlock(); } xsaveUCB(cb);
     ...
     *     ... = <span style="color: #553000">memberWithdrawal</span>.<span style="color: #CC4747">getMember()</span>; <span style="color: #3F7E5E">// you can get by using SetupSelect</span>
     * });
     * </pre>
    ...
    public static class HpSpecification extends HpAbstractSpecification<MemberWithdrawalCQ> {
        ...
        public HpSDRFunction<MemberWithdrawalCB, MemberWithdrawalCQ> myselfDerived() {
            ...
            return cHSDRF(_baseCB, _qyCall.qy(), (String fn, SubQuery<MemberWithdrawalCB> sq, MemberWithdrawalCQ cq, String al, DerivedReferrerOption op)
     ...
     */
    public void orScopeQuery(OrQuery<MemberWithdrawalCB> orCBLambda) {
        xorSQ((MemberWithdrawalCB)this, orCBLambda);
    }
    ...
    protected void xprepareSyncQyCall(ConditionBean mainCB) {
        final MemberWithdrawalCB cb;
        if (mainCB != null) {
            cb = (MemberWithdrawalCB)mainCB;
        } else {
