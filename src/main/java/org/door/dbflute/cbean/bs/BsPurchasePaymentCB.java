...
public class BsPurchasePaymentCB extends AbstractConditionBean {
    ...
    //                                                                           Attribute
    //                                                                           =========
    protected PurchasePaymentCQ _conditionQuery;

    // ===================================================================================
    //                                                                         Constructor
    //                                                                         ===========
    public BsPurchasePaymentCB() {
    ...
    protected PurchasePaymentCQ createLocalCQ() {
        return xcreateCQ(null, getSqlClause(), getSqlClause().getBasePointAliasName(), 0);
    }

    protected PurchasePaymentCQ xcreateCQ(ConditionQuery childQuery, SqlClause sqlClause, String aliasName, int nestLevel) {
        PurchasePaymentCQ cq = xnewCQ(childQuery, sqlClause, aliasName, nestLevel);
        cq.xsetBaseCB(this);
        return cq;
    }

    protected PurchasePaymentCQ xnewCQ(ConditionQuery childQuery, SqlClause sqlClause, String aliasName, int nestLevel) {
     ...
     */
    public void unionAll(UnionQuery<PurchasePaymentCB> unionCBLambda) {
        final PurchasePaymentCB cb = new PurchasePaymentCB(); cb.xsetupForUnion(this); xsyncUQ(cb);
        try { lock(); unionCBLambda.query(cb); } finally { unlock(); } xsaveUCB(cb);
        final PurchasePaymentCQ cq = cb.query(); query().xsetUnionAllQuery(cq);
    }
     ...
     * }).alwaysPresent(<span style="color: #553000">purchasePayment</span> <span style="color: #90226C; font-weight: bold"><span style="font-size: 120%">-</span>&gt;</span> {
     *     ... = <span style="color: #553000">purchasePayment</span>.<span style="color: #CC4747">getPurchase()</span>; <span style="color: #3F7E5E">// you can get by using SetupSelect</span>
     * });
     * </pre>
    ...
    public static class HpSpecification extends HpAbstractSpecification<PurchasePaymentCQ> {
        ...
        public HpSpecification(ConditionBean baseCB, HpSpQyCall<PurchasePaymentCQ> qyCall
         ...
         */
        public HpSDRFunction<PurchasePaymentCB, PurchasePaymentCQ> myselfDerived() {
            assertDerived("myselfDerived"); if (xhasSyncQyCall()) { xsyncQyCall().qy(); } // for sync (for example, this in ColumnQuery)
            return cHSDRF(_baseCB, _qyCall.qy(), (String fn, SubQuery<PurchasePaymentCB> sq, PurchasePaymentCQ cq, String al, DerivedReferrerOption op)
     ...
     */
    public void orScopeQuery(OrQuery<PurchasePaymentCB> orCBLambda) {
    ...
    protected void xprepareSyncQyCall(ConditionBean mainCB) {
        final PurchasePaymentCB cb;
        if (mainCB != null) {
            cb = (PurchasePaymentCB)mainCB;
        } else {
