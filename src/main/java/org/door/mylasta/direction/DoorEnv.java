...
public interface DoorEnv {
    ...
    /** The key of the configuration. e.g. /tmp/lastaflute/door */
    ...
    /** The key of the configuration. e.g. returnpath@door.org */
     ...
     * The value is, e.g. /tmp/lastaflute/door <br>
     ...
     * The value is, e.g. returnpath@door.org <br>
     ...
     * The value is, e.g. door-support@annie.example.com <br>
     * comment: Mail Address for Door Support
     * @return The value of found property. (NotNull: if not found, exception but basically no way)
     */
     ...
     * @author FreeGen
     */
    public static class SimpleImpl extends ObjectiveConfig implements DoorEnv {

        /** The serial version UID for object serialization. (Default) */
        private static final long serialVersionUID = 1L;

        public String getLastaDiSmartDeployMode() {
            return get(DoorEnv.lasta_di_SMART_DEPLOY_MODE);
        }

        public String getDevelopmentHere() {
            return get(DoorEnv.DEVELOPMENT_HERE);
        }

        public boolean isDevelopmentHere() {
            return is(DoorEnv.DEVELOPMENT_HERE);
        }

        public String getEnvironmentTitle() {
            return get(DoorEnv.ENVIRONMENT_TITLE);
        }

        public String getFrameworkDebug() {
            return get(DoorEnv.FRAMEWORK_DEBUG);
        }

        public boolean isFrameworkDebug() {
            return is(DoorEnv.FRAMEWORK_DEBUG);
        }

        public String getTimeAdjustTimeMillis() {
            return get(DoorEnv.TIME_ADJUST_TIME_MILLIS);
        }

        public Long getTimeAdjustTimeMillisAsLong() {
            return getAsLong(DoorEnv.TIME_ADJUST_TIME_MILLIS);
        }

        public String getLogLevel() {
            return get(DoorEnv.LOG_LEVEL);
        }

        public String getLogConsoleLevel() {
            return get(DoorEnv.LOG_CONSOLE_LEVEL);
        }

        public String getLogFileBasedir() {
            return get(DoorEnv.LOG_FILE_BASEDIR);
        }

        public String getMailSendMock() {
            return get(DoorEnv.MAIL_SEND_MOCK);
        }

        public boolean isMailSendMock() {
            return is(DoorEnv.MAIL_SEND_MOCK);
        }

        public String getMailSmtpServerMainHostAndPort() {
            return get(DoorEnv.MAIL_SMTP_SERVER_MAIN_HOST_AND_PORT);
        }

        public String getMailSubjectTestPrefix() {
            return get(DoorEnv.MAIL_SUBJECT_TEST_PREFIX);
        }

        public String getMailReturnPath() {
            return get(DoorEnv.MAIL_RETURN_PATH);
        }

        public String getMailAddressSupport() {
            return get(DoorEnv.MAIL_ADDRESS_SUPPORT);
        }

        public String getJdbcDriver() {
            return get(DoorEnv.JDBC_DRIVER);
        }

        public String getJdbcUrl() {
            return get(DoorEnv.JDBC_URL);
        }

        public String getJdbcUser() {
            return get(DoorEnv.JDBC_USER);
        }

        public String getJdbcPassword() {
            return get(DoorEnv.JDBC_PASSWORD);
        }

        public String getJdbcConnectionPoolingSize() {
            return get(DoorEnv.JDBC_CONNECTION_POOLING_SIZE);
        }

        public Integer getJdbcConnectionPoolingSizeAsInteger() {
            return getAsInteger(DoorEnv.JDBC_CONNECTION_POOLING_SIZE);
        }

        public String getServerDomain() {
            return get(DoorEnv.SERVER_DOMAIN);
        }
    }
}
