...
import org.door.app.logic.i18n.I18nDateLogic;
import org.door.app.web.base.login.DoorLoginAssist;
import org.door.app.web.base.paging.PagingNavi;
import org.door.app.web.base.paging.SearchPagingBean;
 ...
 * @author jflute
 */
public abstract class DoorBaseAction extends TypicalAction // has several interfaces for direct use
        implements LaValidatable<DoorMessages>, DoorHtmlPath {
    ...
    public void hookFinally(ActionRuntime runtime) { // application may override
        if (runtime.isForwardToHtml()) {
            runtime.registerData("headerBean", getUserBean().map(userBean -> {
                return new DoorHeaderBean(userBean);
            }).orElse(DoorHeaderBean.empty()));
        }
    ...
    //                                                                           =========
    @Override
    protected OptionalThing<DoorUserBean> getUserBean() { // to return as concrete class
        return doorLoginAssist.getSessionUserBean(); // #app_customize return empty if login is unused
    }
    ...
    protected OptionalThing<LoginManager> myLoginManager() {
        return OptionalThing.of(doorLoginAssist);
    }
    ...
    @Override
    public ActionValidator<DoorMessages> createValidator() { // for co-variant
        return super.createValidator();
    }

    @Override
    public DoorMessages createMessages() { // application may call
        return new DoorMessages(); // overriding to change return type to concrete-class
    }
    ...
    protected PagingNavi createPagingNavi(PagingResultBean<? extends Entity> page, Object form) { // application may override
        return new PagingNavi(page, op -> {
            op.rangeSize(doorConfig.getPagingPageRangeSizeAsInteger());
            if (doorConfig.isPagingPageRangeFillLimit()) {
                op.fillLimit();
            }
        } , form);
    }
    ...
    protected int getPagingPageSize() { // application may call
        return doorConfig.getPagingPageSizeAsInteger();
    }
